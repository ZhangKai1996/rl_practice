...
class PlanningAgent(object):
    ...
    def visual(self, **kwargs):
        if self.render is None:
            self.render = ValueRender(agent=self)
        self.render.draw(**kwargs)
...
class ValueRender:
    def __init__(self, agent, width=1200, height=1200, padding=0.05):
        self.agent = agent
        self.env = agent.env
        self.video = cv2.VideoWriter(
            filename='figs/value_{}.avi'.format(agent.algo),
    ...
    def draw(self, show=False):
        ...
        base_img_r = self.__draw_r(w_p, h_p, size, border_len)
        base_img_v = self.__draw_v(w_p, h_p, size, border_len)
        base_img_q = self.__draw_q(w_p, h_p, size, border_len)
        base_img = np.hstack([base_img_r, base_img_v, base_img_q])
        base_img = cv2.resize(base_img, (1920, 1080), fx=0.5, fy=0.5)
    ...
    def __draw_r(self, w_p, h_p, size, border_len):
        ...
        for i in range(size):
            ...
            for j in range(size):
                ...
                # Draw start
                if idx == env.start:
                    cv2.circle(base_img, pos, 10, (255, 0, 0), thickness=-1)
                # Write state value
                cv2.putText(
    ...
    def __draw_q(self, w_p, h_p, size, border_len):
        base_img = copy.deepcopy(self.base_img)
        value = self.agent.state_action_value
        env = self.env
        ...
        for i in range(size):
            ...
            for j in range(size):
                ...
                # Draw grids
                cv2.rectangle(base_img, (x1_out, y1_out), (x2_out, y2_out), (0, 0, 0), thickness=2)
                # Draw targets
                if idx in env.targets:
                    cv2.circle(base_img, pos, 10, (0, 255, 0), thickness=-1)
                # Draw start
                ...
                base_img = self.__draw(base_img, [q_act[0], q_act_[0]], max_q, poses, is_max=is_max[0])
                # 1-down
                poses = np.array([(x1_out, y2_out), pos, (x2_out, y2_out)], np.int32)
                base_img = self.__draw(base_img, [q_act[1], q_act_[1]], max_q, poses, is_max=is_max[1])
                # 2-right
                poses = np.array([(x2_out, y1_out), pos, (x2_out, y2_out)], np.int32)
                base_img = self.__draw(base_img, [q_act[2], q_act_[2]], max_q, poses, is_max=is_max[2])
                # 3-left
                poses = np.array([(x1_out, y1_out), pos, (x1_out, y2_out)], np.int32)
                base_img = self.__draw(base_img, [q_act[3], q_act_[3]], max_q, poses, is_max=is_max[3])
                # Draw state
                if i == 0:
                    cv2.putText(
                        base_img, str(j), (int(h_p / 2), row),
    ...
    def __draw(self, base_img, q, max_q, poses, is_max=0):
        # 画三角形
        cv2.polylines(base_img, [poses], True, (0, 0, 0), thickness=1)
        # 填充颜色
        pixel = 0 if is_max else 255
    ...
    def __draw_v(self, w_p, h_p, size, border_len):
        base_img = copy.deepcopy(self.base_img)
        value = self.agent.state_value
        env = self.env
